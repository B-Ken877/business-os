...
describe("church-attendance / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putServiceAttendance(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getServiceAttendance(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listServiceAttendances(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putServiceAttendance(tenantA, entityA2);
    store.putServiceAttendance(tenantB, entityB2);
    expect(store.getServiceAttendance(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getServiceAttendance(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
