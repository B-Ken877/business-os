...
> Attendance for services, events, and programs.

**Component ID:** `church-attendance`
...
Record per-member per-service attendance to track engagement over time.

## Business problem solved

Attendance patterns reveal engagement and potential pastoral needs. Paper tracking makes aggregation impossible.
...
- Record attendance per service/event
- Compute attendance rate per member
- Identify declining attendance
...
- `declineThresholdWeeks` (`number`, default `3`) — Weeks of absence before flagging as declining.
...
Member identity, service date, attendance status. Reveals religious participation; sensitive.
...
- No automatic pastoral follow-up alerts.

## Future improvements

- Automatic pastoral follow-up via notifications-center.
- Service-level attendance summary.
- Visitor tracking.
