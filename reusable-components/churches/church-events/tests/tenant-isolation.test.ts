...
describe("church-events / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putEvent(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getEvent(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listEvents(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putEvent(tenantA, entityA2);
    store.putEvent(tenantB, entityB2);
    expect(store.getEvent(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getEvent(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
