...
describe("church-donations / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putDonation(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getDonation(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listDonations(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putDonation(tenantA, entityA2);
    store.putDonation(tenantB, entityB2);
    expect(store.getDonation(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getDonation(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
