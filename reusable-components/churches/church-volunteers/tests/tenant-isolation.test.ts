...
describe("church-volunteers / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putVolunteer(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getVolunteer(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listVolunteers(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putVolunteer(tenantA, entityA2);
    store.putVolunteer(tenantB, entityB2);
    expect(store.getVolunteer(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getVolunteer(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
