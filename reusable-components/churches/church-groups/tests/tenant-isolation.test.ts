...
describe("church-groups / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putGroup(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getGroup(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listGroups(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putGroup(tenantA, entityA2);
    store.putGroup(tenantB, entityB2);
    expect(store.getGroup(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getGroup(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
