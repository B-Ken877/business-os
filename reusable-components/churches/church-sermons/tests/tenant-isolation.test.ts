...
describe("church-sermons / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putSermon(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getSermon(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listSermons(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putSermon(tenantA, entityA2);
    store.putSermon(tenantB, entityB2);
    expect(store.getSermon(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getSermon(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
