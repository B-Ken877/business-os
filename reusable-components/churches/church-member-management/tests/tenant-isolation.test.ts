...
describe("church-member-management / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putMember(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getMember(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listMembers(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putMember(tenantA, entityA2);
    store.putMember(tenantB, entityB2);
    expect(store.getMember(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getMember(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
