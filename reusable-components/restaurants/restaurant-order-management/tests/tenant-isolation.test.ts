...
describe("restaurant-order-management / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putOrder(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getOrder(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listOrders(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putOrder(tenantA, entityA2);
    store.putOrder(tenantB, entityB2);
    expect(store.getOrder(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getOrder(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
