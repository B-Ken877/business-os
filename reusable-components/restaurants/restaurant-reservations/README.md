...
> Reservations, booking times, customer details, table assignment, and reminders.

**Component ID:** `restaurant-reservations`
...
Accept reservations for future time slots, assign tables, and trigger reminders via notifications-center.

## Business problem solved

Phone-based reservations get double-booked and forgotten. This component makes reservations a queryable, time-bounded entity.
...
- Create a reservation for a date/time/party size
- Assign a table at check-in
- Cancel reservations
...
- `maxReservationsPerDay` (`number`, default `200`) — Cap on reservations per day.
- `reminderLeadMinutes` (`number`, default `60`) — Minutes before reservation to send a reminder.
...
Customer name, phone, party size, reservation time, table assignment. PII; access must be logged.
...
- No automatic table assignment at booking time.
