...
describe("restaurant-ingredient-tracking / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putIngredient(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getIngredient(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listIngredients(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putIngredient(tenantA, entityA2);
    store.putIngredient(tenantB, entityB2);
    expect(store.getIngredient(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getIngredient(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
