...
describe("restaurant-kitchen-display / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putKitchenTicket(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getKitchenTicket(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listKitchenTickets(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putKitchenTicket(tenantA, entityA2);
    store.putKitchenTicket(tenantB, entityB2);
    expect(store.getKitchenTicket(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getKitchenTicket(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
