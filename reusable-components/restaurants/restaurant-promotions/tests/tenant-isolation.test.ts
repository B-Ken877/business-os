...
describe("restaurant-promotions / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putCoupon(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getCoupon(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listCoupons(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putCoupon(tenantA, entityA2);
    store.putCoupon(tenantB, entityB2);
    expect(store.getCoupon(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getCoupon(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
