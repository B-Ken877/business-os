...
describe("restaurant-billing / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putBill(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getBill(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listBills(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putBill(tenantA, entityA2);
    store.putBill(tenantB, entityB2);
    expect(store.getBill(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getBill(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
