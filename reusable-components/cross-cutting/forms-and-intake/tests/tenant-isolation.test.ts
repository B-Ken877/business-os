...
describe("forms-and-intake / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putFormDefinition(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getFormDefinition(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listFormDefinitions(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putFormDefinition(tenantA, entityA2);
    store.putFormDefinition(tenantB, entityB2);
    expect(store.getFormDefinition(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getFormDefinition(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
