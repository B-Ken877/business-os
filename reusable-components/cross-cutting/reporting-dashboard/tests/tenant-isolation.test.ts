...
describe("reporting-dashboard / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putMetric(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getMetric(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listMetrics(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putMetric(tenantA, entityA2);
    store.putMetric(tenantB, entityB2);
    expect(store.getMetric(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getMetric(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
