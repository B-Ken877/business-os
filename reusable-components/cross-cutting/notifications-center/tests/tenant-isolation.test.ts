...
describe("notifications-center / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putNotification(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getNotification(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listNotifications(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putNotification(tenantA, entityA2);
    store.putNotification(tenantB, entityB2);
    expect(store.getNotification(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getNotification(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
