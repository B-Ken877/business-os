...
describe("document-management / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putDocument(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getDocument(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listDocuments(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putDocument(tenantA, entityA2);
    store.putDocument(tenantB, entityB2);
    expect(store.getDocument(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getDocument(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
