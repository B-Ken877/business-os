...
describe("notes-and-comments / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putNote(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getNote(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listNotes(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putNote(tenantA, entityA2);
    store.putNote(tenantB, entityB2);
    expect(store.getNote(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getNote(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
