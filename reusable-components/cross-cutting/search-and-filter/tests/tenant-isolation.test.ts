...
describe("search-and-filter / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putSavedQuery(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getSavedQuery(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listSavedQuerys(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putSavedQuery(tenantA, entityA2);
    store.putSavedQuery(tenantB, entityB2);
    expect(store.getSavedQuery(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getSavedQuery(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
