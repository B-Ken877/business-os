...
describe("school-class-scheduling / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putClassSession(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getClassSession(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listClassSessions(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putClassSession(tenantA, entityA2);
    store.putClassSession(tenantB, entityB2);
    expect(store.getClassSession(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getClassSession(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
