...
describe("school-student-portal / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putStudentPortalSession(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getStudentPortalSession(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listStudentPortalSessions(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putStudentPortalSession(tenantA, entityA2);
    store.putStudentPortalSession(tenantB, entityB2);
    expect(store.getStudentPortalSession(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getStudentPortalSession(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
