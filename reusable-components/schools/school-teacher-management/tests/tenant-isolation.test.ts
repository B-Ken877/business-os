...
describe("school-teacher-management / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putTeacher(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getTeacher(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listTeachers(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putTeacher(tenantA, entityA2);
    store.putTeacher(tenantB, entityB2);
    expect(store.getTeacher(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getTeacher(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
