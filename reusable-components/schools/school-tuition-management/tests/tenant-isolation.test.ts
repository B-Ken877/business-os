...
describe("school-tuition-management / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putTuitionPlan(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getTuitionPlan(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listTuitionPlans(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putTuitionPlan(tenantA, entityA2);
    store.putTuitionPlan(tenantB, entityB2);
    expect(store.getTuitionPlan(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getTuitionPlan(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
