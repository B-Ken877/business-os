...
describe("school-certificates / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putCertificate(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getCertificate(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listCertificates(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putCertificate(tenantA, entityA2);
    store.putCertificate(tenantB, entityB2);
    expect(store.getCertificate(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getCertificate(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
