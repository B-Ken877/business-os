...
> Grades, assessments, report cards, and score history.

**Component ID:** `school-grading`
...
Record per-student per-assessment grades, compute averages, and produce report cards.

## Business problem solved

Grade books on paper are error-prone and slow to aggregate. This component makes grades queryable and report-card generation automatic.
...
- Record grades per assessment
- Compute subject and overall averages
...
- `passingGradePct` (`number`, default `60`) — Passing grade percentage.
...
Student identity, assessment scores, subject averages. Academic records — sensitive, especially for minors.
...
- No subject-level averaging yet.
- No report card PDF generation.

## Future improvements

- Subject-level averaging.
- Report card PDF via document-management.
