...
describe("school-attendance / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putAttendanceRecord(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getAttendanceRecord(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listAttendanceRecords(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putAttendanceRecord(tenantA, entityA2);
    store.putAttendanceRecord(tenantB, entityB2);
    expect(store.getAttendanceRecord(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getAttendanceRecord(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
