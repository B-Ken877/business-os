...
describe("school-parent-communication / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    const entityA = {
      ...
      body: "value",
      direction: "outbound",
      messagingMessageId: undefined,
      createdAt: "2024-01-01T00:00:00Z",
      updatedAt: "2024-01-01T00:00:00Z",
    } as any;
    store.putParentMessage(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getParentMessage(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listParentMessages(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putParentMessage(tenantA, entityA2);
    store.putParentMessage(tenantB, entityB2);
    expect(store.getParentMessage(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getParentMessage(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
