...
describe("retail-promotions / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putPromotion(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getPromotion(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listPromotions(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putPromotion(tenantA, entityA2);
    store.putPromotion(tenantB, entityB2);
    expect(store.getPromotion(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getPromotion(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
