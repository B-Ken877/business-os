...
describe("retail-product-catalog / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putCategory(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getCategory(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listCategorys(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putCategory(tenantA, entityA2);
    store.putCategory(tenantB, entityB2);
    expect(store.getCategory(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getCategory(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
