...
describe("retail-point-of-sale / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putSale(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getSale(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listSales(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putSale(tenantA, entityA2);
    store.putSale(tenantB, entityB2);
    expect(store.getSale(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getSale(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
