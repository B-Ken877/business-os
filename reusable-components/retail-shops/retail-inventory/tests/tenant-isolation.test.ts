...
describe("retail-inventory / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putStockLevel(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getStockLevel(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listStockLevels(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putStockLevel(tenantA, entityA2);
    store.putStockLevel(tenantB, entityB2);
    expect(store.getStockLevel(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getStockLevel(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
