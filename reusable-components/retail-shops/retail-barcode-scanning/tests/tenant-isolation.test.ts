...
describe("retail-barcode-scanning / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putBarcode(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getBarcode(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listBarcodes(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putBarcode(tenantA, entityA2);
    store.putBarcode(tenantB, entityB2);
    expect(store.getBarcode(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getBarcode(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
