...
describe("retail-supplier-management / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putSupplier(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getSupplier(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listSuppliers(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putSupplier(tenantA, entityA2);
    store.putSupplier(tenantB, entityB2);
    expect(store.getSupplier(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getSupplier(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
