...
describe("retail-stock-alerts / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putStockAlert(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getStockAlert(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listStockAlerts(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putStockAlert(tenantA, entityA2);
    store.putStockAlert(tenantB, entityB2);
    expect(store.getStockAlert(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getStockAlert(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
