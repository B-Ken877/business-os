...
describe("clinic-triage / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    const entityA = {
      ...
      urgency: "low",
      classifiedByStaffId: "value",
      createdAt: "2024-01-01T00:00:00Z",
      updatedAt: "2024-01-01T00:00:00Z",
    } as any;
    store.putTriageEntry(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getTriageEntry(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listTriageEntrys(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putTriageEntry(tenantA, entityA2);
    store.putTriageEntry(tenantB, entityB2);
    expect(store.getTriageEntry(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getTriageEntry(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
