...
describe("clinic-reminders / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putReminder(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getReminder(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listReminders(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putReminder(tenantA, entityA2);
    store.putReminder(tenantB, entityB2);
    expect(store.getReminder(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getReminder(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
