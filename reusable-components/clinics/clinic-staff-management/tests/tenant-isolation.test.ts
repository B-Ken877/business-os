...
describe("clinic-staff-management / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putStaff(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getStaff(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listStaffs(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putStaff(tenantA, entityA2);
    store.putStaff(tenantB, entityB2);
    expect(store.getStaff(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getStaff(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
