...
describe("clinic-medical-records / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putMedicalRecord(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getMedicalRecord(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listMedicalRecords(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putMedicalRecord(tenantA, entityA2);
    store.putMedicalRecord(tenantB, entityB2);
    expect(store.getMedicalRecord(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getMedicalRecord(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
