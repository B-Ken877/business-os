...
describe("service-job-tracking / tenant isolation", () => {
  it("store returns undefined when reading another tenant's entity", () => {
    ...
    store.putJob(tenantA, entityA);
    // Tenant B tries to read tenant A's entity.
    const leaked = store.getJob(tenantB, asEntityId("e-1"));
    expect(leaked).toBeUndefined();
    // And the listing for tenant B must not contain tenant A's entity.
    const listB = store.listJobs(tenantB);
  ...
  it("store overwriting in tenant B does not affect tenant A", () => {
    ...
    store.putJob(tenantA, entityA2);
    store.putJob(tenantB, entityB2);
    expect(store.getJob(tenantA, asEntityId("e-shared"))).toBe(entityA2);
    expect(store.getJob(tenantB, asEntityId("e-shared"))).toBe(entityB2);
  });
});
