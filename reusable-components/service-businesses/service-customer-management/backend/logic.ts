/**
 * Business logic for the service-customer-management component.
 *
 ...
 */
export interface ServiceCustomerManagementStore {
...
export class InMemoryServiceCustomerManagementStore implements ServiceCustomerManagementStore {
...
export interface Dependencies {
  readonly store: ServiceCustomerManagementStore;
...
export function createCustomer(
  ...
  deps.permissions.require(ctx, asPermission("service.customers.create"));
  ...
  const v = validated.value;
    ...
    const customer: Customer = {
      id, tenantId: ctx.tenantId, name: v.name, phone: v.phone ?? null,
      email: v.email ?? null, address: v.address ?? null,
      preferencesJson: "{}", createdAt: now, updatedAt: now,
    };
    deps.store.putCustomer(ctx.tenantId, customer);
    deps.audit.record(createAuditEntry({
      tenantId: ctx.tenantId, actorUserId: ctx.userId, componentId: "service-customer-management",
      action: "service.customer.created", entityType: "customer", entityId: id,
      details: { name: v.name },
    }));
    return ok(customer);
}
...
//////////////////////////////////////////////////////////////////////
// setPreferences — Set the customer's preferences JSON.
//////////////////////////////////////////////////////////////////////
export function setPreferences(
  ...
  deps.permissions.require(ctx, asPermission("service.customers.update"));
  const validated = validateSetPreferencesInput(input);
  if (!validated.ok) return validated;
  const v = validated.value;
    try { JSON.parse(v.preferencesJson); } catch {
      return err(ErrorCode.INVALID_INPUT, "preferencesJson is not valid JSON");
    }
    const id = asEntityId(v.customerId);
    const existing = deps.store.getCustomer(ctx.tenantId, id);
    if (!existing) return err(ErrorCode.NOT_FOUND, "customer not found");
    assertSameTenant(ctx, existing.tenantId);
    const updated: Customer = {
      ...existing, preferencesJson: v.preferencesJson, updatedAt: new Date().toISOString(),
    };
    deps.store.putCustomer(ctx.tenantId, updated);
    deps.audit.record(createAuditEntry({
      tenantId: ctx.tenantId, actorUserId: ctx.userId, componentId: "service-customer-management",
      action: "service.customer.preferences_set", entityType: "customer", entityId: id,
      details: {},
    }));
    return ok(updated);
}
